...
X=as.integer(W+1) # define Xs numerical variable
X1 <- as.numeric(X==1)
X2 <- as.numeric(X==2)
X3 <- as.numeric(X==3)
X4 <- as.numeric(X==4)
...
ggplot(PO_DF, aes(x=X, y=Y0)) +
  ...
  #geom_point(color = "red", shape=1, aes(x=X, y=Y1))
  
# RANDOMLY ASSIGN A TREATMENT INDICATOR
# subjects are arriving to be assigned tx or control
# equal to 1 if cond't in outer brackets is true, draw random # 1 if >.5, 0 <.5
