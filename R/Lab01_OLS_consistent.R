...
# y + pop model, 5 = beta0, 1.5 = B1 then create dataframe
Y <- 5 + 1.5*X1 + u
population_data <- data.frame(X1, Y)

# OLS estimation, full sample, huxreg creates a table similar to kable
...
# create dummy df w/ 10000 rows and 2 columns - place to store each est of b0 & b1
betahat_output <- matrix(ncol = 2, nrow = bigN)

#forloop - extract full pop df a sample, go from row 1 to current value of n
